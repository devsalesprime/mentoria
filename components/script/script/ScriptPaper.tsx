...
import type { Bloco, Documento, Fala, PassoDoc, ScriptDoc } from './parseScript';
import { falaParaCopiar } from './parseScript';
...
/** Texto com as instrucoes entre colchetes viradas em etiquetas: [Pausa.], [FALA DO VENDEDOR], [ACIONAR MENTORA]. */
export function comTags(texto: string): React.ReactNode {
  const partes = (texto || '').split(TAG_RE).filter((p) => p !== '');
  return partes.map((p, i) => {
    if (/^\[[^\]]+\]$/.test(p)) {
      const dentro = p.slice(1, -1).trim();
      const acionar = /^ACIONAR/i.test(dentro);
      return <span key={i} className={`script-tag${acionar ? ' script-tag-acionar' : ''}`}>{dentro}</span>;
    }
...
const FalaCard: React.FC<{ fala: Fala; passo: number }> = ({ fala, passo }) => (
  <figure className="script-fala">
    ...
    <blockquote className="script-fala-texto mt-1">{comTags(fala.texto)}</blockquote>
...
}> = ({ documento, ativo, multiplos, refFor, comentariosDo }) => (
...
export const ScriptPaper: React.FC<{
  ...
  refFor: (key: string) => (el: HTMLElement | null) => void;
  comentariosDo: (passo: number) => React.ReactNode;
}> = ({ doc, clubNome, versao, escritoEm, aprovadoEm, docAtivo, refFor, comentariosDo }) => {
  ...
  return (
    <article id="script-print-root" className="script-paper w-full max-w-[760px] mx-auto rounded-2xl px-5 py-7 sm:px-10 sm:py-10 shadow-2xl">
      ...
      {doc.documentos.map((d) => (
        <DocumentoView key={d.id} documento={d} ativo={d.id === docAtivo} multiplos={multiplos} refFor={refFor} comentariosDo={comentariosDo} />
      ))}

      {doc.mapa && (
        <section className="script-mapa mt-10">
          <h2 className="script-h2 font-serif text-2xl text-prosperus-navy-panel mb-3">{doc.mapa.titulo}</h2>
          <div className="script-table script-md" dangerouslySetInnerHTML={{ __html: doc.mapa.html }} />
        </section>
      )}

      {doc.cartao && (
        <section id="script-cartao" className="script-cartao mt-8 scroll-mt-20 lg:scroll-mt-4">
          <div className="flex items-start justify-between gap-3">
            <h2 className="font-serif text-2xl text-prosperus-gold-light leading-tight">Cartão de bolso</h2>
            <CopyButton texto={doc.cartao.texto} rotulo="Copiar cartão" ariaLabel="Copiar cartão de bolso" className="script-copiar-claro" />
          </div>
          <div className="script-cartao-corpo mt-2" dangerouslySetInnerHTML={{ __html: doc.cartao.html }} />
        </section>
      )}
    </article>
  );
