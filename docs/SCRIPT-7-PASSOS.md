...
| `POST /api/script/ficha/gerar-script` | "Pedir nova versão": mesmo job `script` (dedupe por clube). Só com `ficha_status = confirmada`; senão 400 `{ message, faltam[] }`. Resposta `{ job }`. |
...
| `script` | `POST /api/script/ficha/complete` e `POST /api/script/ficha/gerar-script` (1 ativo por clube) | `{ nome, motivo: 'complete' \| 'gerar-script', pedido_em }` | `GET :id/ficha` (usa `valores` + `contexto`) → escreve o script em markdown com `## Passo N` (N = 1..7) → `PUT :id/script` → `PATCH done { result: { versao } }` → aviso no WhatsApp com a `url` devolvida. |
...
|--|--|--|
| `POST /api/jobs/next` | `{ tipo?: 'prefill' \| 'script' \| 'refinar' \| 'any' }` (ausente ou `any` = qualquer tipo; outro valor → 400) | **200** `{ success, job, club: { slug, nome, ativo }, pessoa: { email, nome, notify_phone }, app_url }` com o job **mais antigo em `queued`** (do tipo pedido, ou de qualquer tipo) já marcado `running` (`started_at` = agora, `attempts + 1`). `job.tipo` e `job.payload` dizem o que fazer. Claim atômico em 1 statement (`UPDATE ... WHERE id = (SELECT id ... ORDER BY created_at LIMIT 1) AND status = 'queued' RETURNING *`): 5 workers em paralelo nunca pegam o mesmo job. **204** sem body quando a fila está vazia. `app_url` = `APP_URL` do ambiente ou `protocolo://host` da requisição. |
| `GET /api/jobs?status=&tipo=&limit=` | `status` em `queued|running|done|error|needs_human` (opcional; inválido → 400); `tipo` em `prefill|script|refinar` (opcional); `limit` 1 a 1000 (default 200) | `{ success, data: job[] }` ordenado por `running`, `queued`, `needs_human`, `error`, `done` e depois `created_at DESC`. |
...
| `PUT /api/jobs/:id/script` | `{ content_md (obrigatório, até 2 MB), resumo?, meta? }` | Grava uma **nova versão** do script do clube em `script_versions` (`versao = max + 1`, status `rascunho`, `job_id`). O markdown deve ter os passos como `## Passo N: ...` (N = 1..7): a página do membro divide por esses cabeçalhos para comentar por passo. **200** `{ success, versao, id, status: 'rascunho', url, warnings[] }`; `url` = `<app_url>/prosperus-mentor-diagnosis/dashboard/script` (para o aviso no WhatsApp). Avisa (não bloqueia) travessão e a palavra "diagnóstico". 404 se o clube sumiu. |
...
| Rota | Body | Resposta |
|--|--|--|
| `GET /api/script/versoes` | | `{ success, versoes: [{ id, versao, status, resumo, meta, job_id, aprovado_em, aprovado_por, created_at, comentarios_count }] (mais recente primeiro, sem conteúdo), job (último job script do clube, ou null), ficha_status }`. |
...
npm test                     # vitest: inclui tests/utils/scriptFields.test.ts, tests/components/FichaField.test.tsx,
                             ...
                             #   CRUD do contexto, versões max + 1 e comentários), cohortJobs (escopo de dedupe por tipo, claim "any") e
                             ...
                             #   versões/comentários/aprovar do membro, admin com versões e tipo na fila)
