...
| `PUT /api/jobs/:id/campo` | `{ field_key, sugerido, classe: 'Fato'\|'DER'\|'VZ', fonte, alternativas?: [{ sugerido, fonte }] (máx. 2), nota_interna? }` | Nova sugestão para **um** campo (job `refinar`). Coloca o campo em `sugerido` com o texto novo **mesmo que estivesse decidido**: o valor anterior do mentor vai para `alternativas[0]` com `fonte: "sua versão anterior"` (ele volta com 1 toque); se não estava decidido mas tinha sugestão diferente, ela fica como `"sugestão anterior"`; depois entram as alternativas do worker (até 3 no total). Aplica a regra "sem a definir" (§2.1): placeholder → campo `vazio`, `limpo: true`, texto em `nota_interna`. Ficha `confirmada` com campo decidido reaberto volta para `em_revisao`. `atualizado_por = "worker:<job_id>"`. **200** `{ success, field_key, reaberto, limpo, ficha_status, campo (forma do admin, com nota_interna), warnings[] }`; 400 campo desconhecido ou body fora do schema. Não mexe no status do job: feche com o `PATCH`. |
